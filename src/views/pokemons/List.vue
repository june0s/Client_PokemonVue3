<template>
  <h1>Users</h1>
  <router-link to="/pokemons/add" class="btn btn-sm btn-success mb-2"
      ...
      <template v-if="pokemons.length > 0">
        ...
        <tr v-for="pokemon in pokemons" :key="pokemon.id">
          ...
          <td style="white-space: nowrap">
            ...
            >
            <button
              @click="pokemonsStore.delete(pokemon.id)"
              class="btn btn-sm btn-danger btn-delete-user"
              :disabled="pokemon.isDeleting"
            >
              <span
                v-if="pokemon.isDeleting"
      ...
      <tr v-if="pokemons.error">
        <td colspan="4">
          <div class="text-danger">
            Error loading pokemons: {{ pokemons.error }}
          </div>
        </td>
      </tr>
    </tbody>
